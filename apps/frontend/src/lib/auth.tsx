...
type AuthContextValue = {
  ...
  signup: (email: string, password: string, name?: string, dateOfBirth?: string, phone?: string) => Promise<void>;
  signout: () => void;
};
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const onStorage = () => {
      try { const raw = localStorage.getItem('user'); setUser(raw ? JSON.parse(raw) : null); } catch(e){}
    };
  ...
  const signup = async (email: string, password: string, name?: string, dateOfBirth?: string, phone?: string) => {
    loader.show('Signing up...');
    try {
      const res = await api.post('/auth/signup', { email, password, name, dateOfBirth, phone });
      const data = res.data;
      if (data && data.accessToken) {
        persistTokens(data.accessToken, data.user);
