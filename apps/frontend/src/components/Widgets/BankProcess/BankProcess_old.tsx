...
const BankProcess: React.FC = () => {
  ...
  const reportTxAndStartPoll = async (txHash: string, to: string, amountDecimal: number | string) => {
    ...
    while (attempt < maxAttempts && !reported) {
      attempt++;
      try {
        const r = await api.post('/deposits/direct', { txHash, contract: TOKEN_CONTRACT, toAddress: to, amount: amountDecimal });
      ...
      {tab === 0 && (
        <div className='box-content box-horizontal-padding box-vertical-padding box-content-height-nobutton'>
          ...
          <div className='box-vertical-padding'>
            ...
            <input id='amount' type='number' min='1' placeholder='100' value={''} />
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const walletEl = document.getElementById('wallet') as HTMLInputElement;
                    const amount = Number(amountEl?.value || 100);
                    const walletAddr = walletEl?.value || undefined;

                    const payload: any = { amount: amount.toString(), currency: 'USDT-TRON' };
                    ...
                    let recipient: string | null = result?.walletAddress || null;
                    ...
                    if (!recipient) {
                      ...
                      try {
                        ...
                        if (sr.data && sr.data.ok && sr.data.address) recipient = sr.data.address;
                      } catch(e){ /* ignore */ }
                    }

                    if (!recipient) {
                      setTxStatus(null);
                      setOutput('No on-chain recipient available. Please use manual checkout/QR.');
                      return;
                    }
                    ...
                    if (!txHash) {
                      setTxStatus(null);
                      setOutput('Transaction not submitted or user rejected the request.');
                      return;
                    }

                    setTxStatus('reporting-tx');
                    setOutput(`Reporting tx ${txHash} to server...`);
                    const report = await reportTxAndStartPoll(txHash, recipient, amount);
                    setTxStatus('reported');
                    setOutput('Transaction reported — server response: ' + JSON.stringify(report));

                    // save to history
                    saveToHistory({ depositId: report?.depositId || result?.depositId || null, invoiceId: result?.invoiceId || null, tx: txHash, txLink: (window as any)._NETWORK === 'mainnet' ? `https://tronscan.org/#/transaction/${txHash}` : `https://shasta.tronscan.org/#/transaction/${txHash}`, createdAt: new Date().toISOString() });
                  } catch (err: any) {
                    setTxStatus(null);
                    setOutput('Auto-send failed: ' + (err?.response?.data?.message || err?.message || String(err)));
                  }
                }}
              >
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (walletAddr) payload.walletAddress = walletAddr;
                              ...
                              const depositId = res.data?.depositId || res.data?.id || null;
                              const paymentUrl = res.data?.paymentUrl || res.data?.checkout || res.data?.checkoutLink || null;
                              if (depositId && !walletConnected && paymentUrl) {
                                // longer polling window for manual payments
                                setOutput(`Created manual deposit ${depositId}. Polling for confirmation...`);
                                pollDepositStatus(depositId, 300, 10000);
                              }
                      } catch (err: any) {
                        setOutput('Deposit failed: ' + (err?.response?.data?.message || err.message));
                      }
                    }}
                  >
                    Create Deposit
            ...
            <div style={{ marginTop: 12 }}>
              ...
              {txStatus && <div><strong>TX Status:</strong> {txStatus}</div>}
            </div>
            {depositResult && (
              <div style={{ marginTop: 12, background: 'var(--surface)', padding: 12, borderRadius: 6 }}>
                <div>
                  <strong>Invoice:</strong> {depositResult.invoiceId || depositResult.invoice || '-'}
                </div>
                <div>
                  <strong>Deposit ID:</strong> {depositResult.depositId || depositResult.id || '-'}
                </div>
                <div>
                  <strong>Payment URL:</strong>{' '}
                  {depositResult.paymentUrl || depositResult.checkout || depositResult.checkoutLink ? (
                    <a href={depositResult.paymentUrl || depositResult.checkout || depositResult.checkoutLink} target='_blank' rel='noreferrer'>
                      Open payment link
                    </a>
                  ) : (
                    <span className='address'>{depositResult.walletAddress || 'No payment URL provided'}</span>
                  )}
                </div>
                {(depositResult.paymentUrl || depositResult.checkout || depositResult.checkoutLink) && (
                  <div style={{ marginTop: 8 }}>
                    <QRCode value={depositResult.paymentUrl || depositResult.checkout || depositResult.checkoutLink} size={128} />
                  </div>
                )}
                <div style={{ marginTop: 8 }}>
                  <button
              ...
              {historyList.length > 0 ? (
                <ul>
                  {historyList.map((item, idx) => (
                    <li key={idx}>
                      Deposit {item.depositId || item.id} - Invoice: {item.invoiceId} - TX: {item.tx ? <a href={item.txLink} target='_blank' rel='noreferrer'>{item.tx.slice(0,10)}...</a> : 'N/A'}
                    </li>
                  ))}
                </ul>
              ) : <p>No history</p>}
              ...
              {publicList.length > 0 ? (
                <ul>
                  {publicList.map((item, idx) => (
                    <li key={idx}>
                      Deposit {item.id} - Status: {item.status} - Amount: {item.amount}
                    </li>
                  ))}
                </ul>
