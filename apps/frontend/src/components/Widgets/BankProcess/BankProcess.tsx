...
import { Select, TextInput, Button, Alert, Tooltip, Modal, ActionIcon } from '@mantine/core';
import { IconCurrencyDollar, IconCoin, IconCopy, IconExternalLink, IconWallet, IconInfoCircle, IconCheck, IconAlertCircle, IconCurrencyBitcoin, IconLoader } from '@tabler/icons-react';
...
const BankProcess: React.FC = () => {
  ...
  const [lottieType, setLottieType] = useState<'success' | 'failed' | null>(null);
      ...
      for (let i = 0; i < attempts; i++) {
        try {
          ...
          if (res && res.data && res.data.status === 'CONFIRMED') {
            ...
            setOutput(`✅ Deposit ${res.data.depositId || res.data.id} confirmed on-chain and settled.`);
            try { notify.success('Deposit confirmed and settled'); } catch (e) { /* ignore */ }
            return res.data;
          } else {
            if (res && res.data) setDepositResult(res.data);
            // keep user informed
            setOutput(`ℹ️ Deposit ${depositId} status: ${res?.data?.status || 'unknown'}`);
          }
        } catch (e) {
          // ignore
        }
  ...
  const reportTxAndStartPoll = async (txHash: string, to: string, amountDecimal: number | string) => {
    ...
    try { setLastReportAttempt({ txHash, to, amount: amountDecimal }); } catch (e) { /* ignore */ }
    const maxAttempts = 6;
    let attempt = 0;
    let reported = false;
    ...
    try {
      ...
      setOutput(`❌ Failed to report tx to server: ${typeof body === 'string' ? body : JSON.stringify(body)}`);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const fetchAddr = async () => {
      try {
        ...
        if (!mounted) return;
        if (res?.data && res.data.ok && res.data.address) setStoreTronAddress(res.data.address);
      } catch (e) {
        // ignore
      }
    };
  ...
  const qrValue = (() => {
    const pv = depositResult?.paymentUrl || depositResult?.checkout || depositResult?.checkoutLink;
    if (pv) return pv;
    // try deposit result wallet addr first, then the store default address
    const addr = depositResult?.walletAddress || depositResult?.address || storeTronAddress || null;
  ...
  const getLogoDataUrl = (currency: string | undefined | null) => {
    try {
      ...
      const surfaceCol = rootStyles?.getPropertyValue('--surface')?.trim() || '#f0f0f0';
      const textCol = rootStyles?.getPropertyValue('--text')?.trim() || '#222';
      const contrastCol = rootStyles?.getPropertyValue('--primary-contrast')?.trim() || '#ffffff';
      const usdtCol = rootStyles?.getPropertyValue('--usdt')?.trim() || '#00b638';
      ...
      if (cur.toLowerCase().includes('btc')) {
        const iconSvg = renderToStaticMarkup(<IconCurrencyBitcoin size={20} strokeWidth={1.5} stroke={contrastCol} />);
  ...
  const handleCreateDeposit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!recipient && paymentUrl) {
        try { const u = new URL(paymentUrl); recipient = u.searchParams.get('address') || u.searchParams.get('recipient') || u.searchParams.get('to') || null; } catch (err) { /* ignore */ }
      }
      if (!recipient) {
        try { const sr = await api.get('/deposits/store/current/tron-address'); if (sr.data && sr.data.ok && sr.data.address) recipient = sr.data.address; } catch (err) { /* ignore */ }
      }
      ...
      if (walletConnected && recipient) {
        ...
        try {
          ...
          if (!txHash) {
            setOutput('⚠️ Transaction was not submitted (user rejected or provider failed). No polling will start.');
            return;
          }
          setOutput(`Transaction sent to ${recipient} (${txHash}), reporting to server and waiting for settlement...`);
          ...
          try {
            const reportResp = await reportTxAndStartPoll(txHash, recipient, amount);
            ...
            if (reportResp && typeof reportResp === 'object') {
              ...
              const serverDeposit = reportResp.depositId || reportResp.id ? reportResp : (reportResp.body || reportResp);
              setDepositResult(serverDeposit || reportResp);
              saveToHistory({ txHash, to: recipient, amount, depositId: serverDeposit?.depositId || serverDeposit?.id || null, createdAt: new Date().toISOString() });
            }
          } catch (e) {
      ...
      const msg = error?.response?.data?.message || error?.message || 'Deposit creation failed';
      notify.error(msg);
  ...
  return (
    <div style={{ fontFamily: 'Poppins, sans-serif', color: 'var(--text)' }}>
      <Modal opened={invoiceModalOpen} onClose={() => setInvoiceModalOpen(false)} title="Invoice preview" size="lg">
        {(() => {
          const url = depositResult?.paymentUrl || depositResult?.checkout || depositResult?.checkoutLink;
          if (url) {
            return (
              <div style={{ height: 520 }}>
                <iframe src={url} title="invoice-preview" style={{ width: '100%', height: '100%', border: 'none', borderRadius: 8 }} />
              </div>
            );
          }
  ...
  <Modal opened={lottieModalOpen} onClose={() => setLottieModalOpen(false)} title={lottieType === 'success' ? 'Deposit confirmed' : 'Deposit status'} size="sm" centered zIndex={99999999}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 12 }}>
          {lottieType === 'success' && (
            <div aria-hidden style={{ borderRadius: 12, padding: 8, background: hexToRgba(primary, 0.06), pointerEvents: 'none' }}>
              <LottiePlayer animationData={SuccessAnim} loop={false} autoplay style={{ width: 240, height: 240 }} />
            </div>
          )}
          {lottieType === 'failed' && (
            <div aria-hidden style={{ borderRadius: 12, padding: 8, background: hexToRgba(primary, 0.06), pointerEvents: 'none' }}>
              <LottiePlayer animationData={FailedAnim} loop={false} autoplay style={{ width: 240, height: 240 }} />
            </div>
          )}
          <div style={{ marginTop: 8, fontWeight: 700, color: 'var(--primary)' }}>{lottieType === 'success' ? 'Deposit confirmed' : 'Deposit failed'}</div>
        </div>
      </Modal>
      {/* Wallet / network card */}
      <div className={styles.card} style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <div style={{ width: 40, height: 40, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', background: primary, color: contrast }} aria-hidden>
            <IconWallet size={18} color={contrast} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 16, fontWeight: 700 }}>Select Network & Connect Wallet</div>
            <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 6 }}>Connect TronLink to enable auto deposits or create manual invoice</div>
          </div>
        </div>
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  setWalletConnected(true); setWalletAddress(addr); setOutput(`✅ Connected\n${addr}`);
                } catch (e: any) { setOutput(`❌ ${e?.message || e}`); }
              }}
            >
              Connect
            </Button>
      ...
      <div className={styles.unifiedHeader} style={{ marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <div style={{padding: '10px 18px', borderRadius: '999px', fontSize: 24, fontWeight: 600, color: 'var(--text)', border: `1px solid ${hexToRgba(primary, 0.06)}`}}>Deposit</div>
              ...
              <div className={styles.currencyBadgeContainer}>
                <div className={styles.currencyBadge} aria-hidden>
                  {currencySelected === 'USDT' ? (
                      ...
                      <circle cx="12" cy="12" r="11" fill="var(--usdt, #00b638)" />
                      <path d="M8 12.5c0-1.5 3-1.75 4.5-1.75 1.5 0 4.5.25 4.5 1.75s-3 1.75-4.5 1.75C11 14.25 8 14 8 12.5z" fill="var(--primary-contrast)" opacity="0.95" />
                          <path d="M12 7.5v9" stroke="var(--primary-contrast)" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
...
<form onSubmit={handleCreateDeposit} className={styles.depositForm} style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 12 }}>
  {/* Row 1: Currency (with icon) + Amount inline */}
  <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
    {/* Currency control (inline - label removed, use placeholder) */}
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 220, flex: '0 0 45%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%' }}>
        {/* visual icon left */}
                {/* visual icon left */}
                <div style={{ width: 38, height: 38, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--surface)', border: `1px solid ${hexToRgba(primary, 0.06)}` }}>
                ...
                <Select
                  ...
                  name="currency"
                  defaultValue="USDT"
                  data={[{ value: 'USDT', label: 'USDT • Tether' }, { value: 'BTC', label: 'BTC • Bitcoin' }]}
                  placeholder="USDT • Tether (default)"
                  aria-label="Select payment currency"
                  size="md"
                  radius="md"
                  className={styles.mantineField}
                  styles={{ input: { height: 44, fontSize: 14, paddingLeft: 12 } }}
  ...
  <div style={{ flex: '1', minWidth: 180 }}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <div style={{ width: 34, height: 34, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--surface)', border: `1px solid ${hexToRgba(primary, 0.06)}` }}>
                  <IconCoin size={14} />
                </div>
                <TextInput
                  id="amount"
  ...
  <div style={{ padding: '6px 10px', borderRadius: 8, background: 'var(--surface)', border: `1px solid ${hexToRgba(primary, 0.06)}`, fontSize: 13, color: 'var(--text)' }}>
          {currencySelected}
        </div>
      </div>
    </div>
  ...
  {/* Row 3: Actions (primary + secondary) */}
      ...
      <Button type="submit" radius="md" size="md" style={{ width: '100%', maxWidth: 420, minWidth: 180, margin: '0 auto', display: 'block' }}>
        {walletConnected ? 'Create Deposit' : 'Create Deposit (Manual)'}
      </Button>
  ...
  <div className={styles.tilesRow}>
    <div className={`${styles.infoTile} ${depositResult?.invoiceId ? styles.hasData : ''}`}>
      <div style={{ fontSize: 12, color: 'var(--muted)', fontWeight: 600 }}>Invoice</div>
      <div className={styles.tileValueRow}>
        <Tooltip label={depositResult?.invoiceId || '-'} withArrow disabled={!depositResult?.invoiceId || (depositResult.invoiceId || '').length <= 28}>
          <div className={styles.tileValue} style={{ cursor: 'text' }}>{depositResult?.invoiceId || '-'}</div>
        </Tooltip>
        {depositResult?.invoiceId && (
          <button
            ...
            onClick={() => {
              try { navigator.clipboard?.writeText(String(depositResult.invoiceId)); notify.success('Invoice copied'); } catch (e) { notify.error('Copy failed'); }
            }}
    ...
    <div className={`${styles.infoTile} ${depositResult?.depositId ? styles.hasData : ''}`}>
      <div style={{ fontSize: 12, color: 'var(--muted)', fontWeight: 600 }}>Deposit ID</div>
      <div className={styles.tileValueRow}>
        <Tooltip label={depositResult?.depositId || depositResult?.id || '-'} withArrow disabled={!depositResult?.depositId && !depositResult?.id}>
          <div className={styles.tileValue} style={{ cursor: 'text' }}>{depositResult?.depositId || depositResult?.id || '-'}</div>
        </Tooltip>
        {(depositResult?.depositId || depositResult?.id) && (
          <button
            ...
            className={styles.tileCopyBtn}
            onClick={() => {
              const v = depositResult?.depositId || depositResult?.id || '';
              try { navigator.clipboard?.writeText(String(v)); notify.success('Deposit ID copied'); } catch (e) { notify.error('Copy failed'); }
            }}
    ...
    <div className={`${styles.infoTilePrimary} ${depositResult?.status ? styles.hasData : ''}`}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', width: '100%' }}>
        ...
        <div aria-hidden>
          {(() => {
            ...
            if (st === 'CONFIRMED') return <IconCheck size={16} color="var(--primary)" />;
            if (st === 'PENDING' || st === 'NEW' || st === 'ONCHAIN') return <IconLoader size={16} className={styles.spin} />;
            if (st === 'FAILED' || st === 'ERROR' || st === 'report-failed') return <IconAlertCircle size={16} color="var(--primary)" />;
            return <IconInfoCircle size={16} />;
          })()}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', justifyContent: 'space-between' }}>
          <div className={styles.tileValue} style={{ fontWeight: 700 }}>{depositResult?.status || '-'}</div>
            {txStatus && (txStatus.includes('poll') || txStatus.includes('waiting') || txStatus.includes('retry')) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <IconLoader size={16} className={styles.spin} />
              <span style={{ fontSize: 12, color: 'var(--muted)' }}>waiting...</span>
            </div>
          )}
        </div>
      </div>
  ...
  if (st === 'CONFIRMED') { icon = <IconCheck size={16} color="var(--primary)" />; color = 'blue'; }
  else if (st === 'PENDING' || st === 'NEW' || st === 'ONCHAIN') { icon = <IconLoader size={16} color="var(--primary)" className={styles.spin} />; color = 'blue'; variant = 'outline'; }
  else if (st === 'FAILED' || st === 'ERROR' || st === 'report-failed') { icon = <IconAlertCircle size={16} color="var(--primary)" />; color = 'blue'; }
      ...
      return (
          ...
          variant={variant}
          aria-live="polite"
          className={`${styles.statusAlert} ${st === 'CONFIRMED' ? styles.statusSuccess : (st === 'PENDING' || st === 'NEW' || st === 'ONCHAIN') ? styles.statusPending : styles.statusFailed}`}
          style={{ border: st === 'PENDING' || st === 'NEW' || st === 'ONCHAIN' ? `1px solid ${hexToRgba(primary, 0.12)}` : undefined }}
        >
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ flex: 1, color: 'var(--text)' }}>{output}</div>
            {(depositResult?.explorerUrl || depositResult?.txHash || lastReportAttempt) && (
              <div style={{ marginLeft: 12, display: 'flex', gap: 8, alignItems: 'center' }}>
                {(depositResult?.explorerUrl || depositResult?.txHash) && (
                  <button
                    onClick={() => {
                      const url = depositResult?.explorerUrl || (depositResult?.txHash ? ((window as any)._NETWORK === 'mainnet' ? `https://tronscan.org/#/transaction/${depositResult.txHash}` : `https://shasta.tronscan.org/#/transaction/${depositResult.txHash}`) : null);
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        await reportTxAndStartPoll(lastReportAttempt.txHash, lastReportAttempt.to, lastReportAttempt.amount);
                      } catch (e: any) {
                        setOutput(`❌ Retry failed: ${e?.message || String(e)}`);
                      }
                    }}
                    className={styles.ghostBtn}
                ...
                {txStatus === 'report-failed' && (
                  <div style={{ marginLeft: 12, pointerEvents: 'none' }}>
                    <LottiePlayer animationData={FailedAnim} loop={false} autoplay style={{ width: 68, height: 68 }} />
                  </div>
                )}
          </div>
        </Alert>
        ...
        <div className={styles.rightCol}>
          <div className={`${styles.card} ${styles.cardInnerFill}`}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div className={styles.netBadge} aria-hidden>{selectedNetwork}</div>
            </div>
                ...
                <div className={styles.qrContainer}>
                  {qrValue ? (
                    (() => {
                      ...
                      const currencyKey = (depositResult?.currency || currencySelected || 'USDT');
                      const logoSrc = getLogoDataUrl(currencyKey) || null;
                      return <QRWithLogo data={qrData} logoSrc={logoSrc || undefined} size={220} imageSize={0.18} type="svg" />;
                    })()
                  ) : (
                    <div style={{ width: 220, height: 220, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Alert icon={<IconInfoCircle size={16} />} color="blue" variant="light">
                        No invoice yet — create a deposit to generate a QR code or preview an invoice.
                      </Alert>
                    </div>
                  )}
                </div>
            ...
            {/* tiles moved to the left column (under the form) */}
              ...
              {depositResult?.txLink && (
                <Button variant="subtle" onClick={() => window.open(depositResult.txLink, '_blank')} style={{ color: primary }}>
                  <IconExternalLink size={14} />
                  <span style={{ marginLeft: 8 }}>View Transaction</span>
                </Button>
              )}
              <Button variant="outline" onClick={() => { navigator.clipboard?.writeText(depositResult?.walletAddress || ''); notify.success('Address copied'); }} style={{ borderColor: primary, color: primary }}>
                <IconCopy size={14} />
      ...
      <div style={{ marginTop: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <div>
            <button className={styles.ghostBtn} onClick={fetchPublicList}>Refresh</button>
          </div>
        </div>
              ...
              <div>
                {pageItems.map((item: any, idx: number) => (
                  <div key={idx} className={styles.publicItem}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <strong style={{ color: 'var(--primary)' }}>{item.depositId}</strong>
                        <span style={{ padding: '4px 8px', borderRadius: 8, fontSize: 12, background: 'var(--primary)', color: 'var(--primary-contrast)' }}>{item.status}</span>
                    </div>
                    <div style={{ color: 'var(--muted)', fontSize: 13 }}>{item.currency} {item.amount} • {new Date(item.createdAt).toLocaleString()}</div>
                    <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                      {item.txHash && (<a href={((window as any)._NETWORK === 'mainnet') ? `https://tronscan.org/#/transaction/${item.txHash}` : `https://shasta.tronscan.org/#/transaction/${item.txHash}`} target="_blank" rel="noreferrer" style={{ color: 'var(--primary)', textDecoration: 'none', display: 'inline-flex', gap: 6, alignItems: 'center' }} aria-label="View transaction"> <IconExternalLink size={14} /> <span style={{ marginLeft: 4 }}>TX</span></a>)}
                      {item.invoiceId && (<span style={{ fontFamily: 'monospace', color: 'var(--text)' }}>{item.invoiceId}</span>)}
                    </div>
                  </div>
                ))}

                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                  <div style={{ color: 'var(--muted)', fontSize: 13 }}>Showing {start + 1}-{Math.min(start + PAGE_SIZE, total)} of {total}</div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button className={styles.ghostBtn} onClick={() => setRecentPage((p) => Math.max(1, p - 1))} disabled={page <= 1}>Prev</button>
                    <button className={styles.ghostBtn} onClick={() => setRecentPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages}>Next</button>
                  </div>
                </div>
              </div>
            );
          })() : (
            <div style={{ textAlign: 'center', color: 'var(--muted)', padding: 24 }}>Loading recent deposits...</div>
          )}
        </div>
      </div>
    </div>
