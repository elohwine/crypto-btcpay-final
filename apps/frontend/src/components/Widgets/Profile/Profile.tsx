...
const Profile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const filtered = rows.filter((r: any) => supported.includes(String(r.currency).toUpperCase()));
        setDeposits(filtered);
      } catch (err: any) {
        console.warn('Profile: failed to load deposits', err);
        if (!mounted) return;
        setDepositsError(err?.response?.data?.message || err?.message || 'Failed to load deposits');
      } finally {
  ...
  return (
    <Box>
      ...
      <div className='widget-profile box-content box-content-height-nobutton' style={{ position: 'relative', background: `linear-gradient(180deg, ${hexToRgba(primary, 0.04)} 0%, rgba(0,0,0,0) 40%)` }}>
        {/* subtle primary accent strip (reduced) */}
        <div aria-hidden style={{ height: 8, background: hexToRgba(primary, 0.04), borderRadius: 6, margin: '12px', zIndex: 0 }} />
  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: 18, marginBottom: 18, position: 'relative', zIndex: 3 }}>
        ...
        <Indicator position='bottom-end' offset={8} color='green' withBorder>
          <Avatar
            size={125}
            radius='xl'
            src={(user as any)?.avatar || (user as any)?.picture || (user as any)?.image || 'https://e7.pngegg.com/pngimages/799/987/png-clipart-computer-icons-avatar-icon-design-avatar-heroes-computer-wallpaper-thumbnail.png'}
  ...
  <div className='box-horizontal-padding' style={{ marginTop: 12 }}>
          <div className='center'>
            <h3 style={{ margin: 0, marginBottom: 18, color: 'var(--text)' }}>{user?.name || user?.email || 'Guest'}</h3>
          </div>
